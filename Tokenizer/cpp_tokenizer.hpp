        ...
        while (i < source.length()) {
            ...
            if (source[i] == '"' || source[i] == '\'') {
                ...
                while (i < source.length() && source[i] != quote) {
                    if (source[i] == '\\' && i + 1 < source.length()) { i++; column++; }
                    i++; column++;
                }
                if (i < source.length()) { i++; column++; }
            ...
            if (std::isdigit(static_cast<unsigned char>(source[i])) ||
                ...
                size_t start = i;
                while (i < source.length() && (std::isalnum(static_cast<unsigned char>(source[i])) ||
                       source[i] == '.' || source[i] == 'e' || source[i] == 'E' ||
                       source[i] == 'x' || source[i] == 'X' || source[i] == '\'')) {
                    i++; column++;
                }
